...
from src.models.encoders import PatchifyEncoder
from src.models.question_encoders import QuestionEncoder
...
implemented (and fixed) exactly once.
    ...
    VQASyncNetConfig()                       recurrent baseline
    VQASyncNetConfig(readout_mode='sync')     ctm readout
    VQASyncNetConfig(partition='quadrant',
                             readout_mode='sum')      partitioned
    VQASyncNetConfig(partition='views',
                             n_modules=3,
...
class VQASyncNet(nn.Module):
    ...
    def __init__(
        ...
        if enc.pop('name', 'patchify') != 'patchify':
            raise ValueError(
                f'syncnet supports the patchify encoder '
        ...
        self.encoders = nn.ModuleList([
            PatchifyEncoder(img_size, enc_ch, int(enc['patch_size']))
