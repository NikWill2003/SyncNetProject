...
def _one_hot_idx(x: torch.Tensor, start: int, end: int) -> int:
    return int(x[start:end].argmax().item())


def _answer_idx(answer: torch.Tensor | np.ndarray | int) -> int:
    a = _to_cpu_tensor(answer)
