...
def _gen_example(
    ...
    *positions* in negatives are rejection-biased in a way positives are
    ...
    scene_arr = np.array(
        [[SHAPE_TO_IDX[o.shape], o.pos[0], o.pos[1]] for o in scene],
        dtype=np.int64)                              # (num_objects, 3): letter, x, y
    return _draw(scene, img_size), encode_question(x, rel, y), int(label), scene_arr
...
def _gen_split(
    ...
    scenes = np.empty((n, num_objects, 3), dtype=np.int64)
    ...
    for i, (pair, rel, label) in enumerate(schedule):
        ...
        images[i], questions[i], answers[i], scenes[i] = out
...
def prepare_sqoop(data_cfg: 'SqoopDataConfig') -> None:
    ...
    if iid:
        print(f'sqoop rhs={rhs}: IID control -- no held-out pairs, '
              'test_unseen is a second draw over seen pairs')
        rep_val = rep_test = 0
    else:
    ...
    if iid:
        # No pairs are held out, so there is nothing unseen to test on.
        # test_unseen is written as a fresh draw over SEEN pairs, which is
        # what makes this the control: it measures task difficulty with the
        # systematic component removed. Do not read it as a generalisation
        # number.
