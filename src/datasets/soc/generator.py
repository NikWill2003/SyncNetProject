...
def build_dataset(
    ...
    # The shared flat layout: one row per question, with image_idx pointing
    # back at the scene. Images are stored once per scene and gathered via
    # image_idx, so nothing is duplicated.
    ternary_q = np.array(ternary_questions)      # (n_scenes, nb_q, q_dim)
    ...
    # uint8, not int64: these are one-hot flags that every consumer casts
    # to float anyway, and the whole split sits in VRAM.
    if questions.max() <= np.iinfo(np.uint8).max and questions.min() >= 0:
        questions = questions.astype(np.uint8)

    return {
        'images': np.array(imgs),
        'questions': questions,                     # (N, q_dim)
        'answers': answers.astype(np.int64),        # (N,)
        'image_idx': image_idx,                     # (N,)
