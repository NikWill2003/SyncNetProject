...
def fixed_point_signature(traces: dict) -> dict[str, float]:
    """The five-metric signature of the field's trivial fixed point, read
    from a v16 trace dict ({'phases', 'slot_reads', 'anchors', 'field_R'}).

      field_R        mean per-group coherence of the settled field: -> 1 at
                     global synchrony (the trivial attractor)
      read_overlap   mean pairwise cosine of slot read distributions: -> 1
                     when every slot reads the same cells (binding collapse)
      slot_coverage  fraction of slots claiming a non-trivial cell share
                     (< 1/2M of the uniform share counts as unassembled)
      anchor_R       alignment of the M anchors with their mean: -> 1 when
                     the competition found one attractor
      bus_R          order parameter of the bus phases at the last step

    `fixed_point` is the conjunction that, in every diagnosed SQOOP run,
    separated the flatlined seeds from the healthy ones: a synchronised
    field read promiscuously. Reported per eval batch; NaN-free by
    construction (empty keys are simply skipped)."""
    sig: dict[str, float] = {}
    if 'field_R' in traces:
        sig['field_R'] = float(traces['field_R'])
    reads = traces.get('slot_reads')
    if reads is not None:
        rn = torch.nn.functional.normalize(reads, dim=-1)
        M = rn.shape[1]
        ov = torch.einsum('bmp,bnp->bmn', rn, rn)
        off = (ov.sum((1, 2)) - ov.diagonal(dim1=1, dim2=2).sum(-1)) / max(M * (M - 1), 1)
        sig['read_overlap'] = off.mean().item()
        share = reads.sum(-1) / reads.sum((-2, -1), keepdim=False).unsqueeze(-1).clamp_min(1e-8)
        sig['slot_coverage'] = (share > 1.0 / (2 * M)).float().mean().item()
    anchors = traces.get('anchors')
    if anchors is not None:
        a = anchors.flatten(2)
        a = torch.nn.functional.normalize(a, dim=-1)
        sig['anchor_R'] = a.mean(1).norm(dim=-1).mean().item()
    phases = traces.get('phases')
    if phases is not None:
        sig['bus_R'] = phases.mean(1).norm(dim=-1).mean().item()
    if {'field_R', 'read_overlap'} <= sig.keys():
        sig['fixed_point'] = float(sig['field_R'] > 0.95 and sig['read_overlap'] > 0.90)
    return sig
...
class sort_of_clevr_sync_metrics_callback(BaseCallBack):
    ...
    def _compute(
        ...
        if hasattr(model, 'PHASE_OVERRIDES') and not getattr(model, 'is_syncnet', False):
            # v16 component models: read the signature off the ordinary
            # forward trace; no extra passes.
            with torch.inference_mode():
                out = model({'images': images, 'questions': questions,
                             'answers': answers})
            return fixed_point_signature(out.get('traces', {})) or None
